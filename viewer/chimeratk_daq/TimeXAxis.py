...
def getDateString(timeStamp):
  fmt = "%Y-%m-%d %H:%M:%S"
  t = datetime.fromtimestamp(timeStamp[0])
  return t.strftime(fmt) + "." + str(timeStamp[1]).zfill(3)
...
class DateAxisItem(AxisItem):
    ...
    def tickValues(self, minVal, maxVal, size):
        ...
        L = len(majticks)
        # Somehow the width of the plot is not correct -> that results in maxMajSteps=0
#         if L > maxMajSteps:
        if maxMajSteps > 0 and L >  maxMajSteps:
...
#         self.setParentItem(plotItem)
        ...
        if self._oldAxis == None:
          self._oldAxis = self.plotItem.axes[self.orientation]['item']
        self._oldAxis.hide()
        self.plotItem.axes[self.orientation]['item'] = self
        pos = self.plotItem.axes[self.orientation]['pos']
        self.plotItem.layout.addItem(self, *pos)
        self.setZValue(-1000)
        self.show()
    ...
    def detachFromPlotItem(self):
        ...
        if self._oldAxis != None:
          ...
          self._oldAxis.setZValue(-1000)
          self._oldAxis.show()
          self.plotItem.axes[self._oldAxis.orientation]['item'] = self._oldAxis
          pos = self.plotItem.axes[self._oldAxis.orientation]['pos']
          self.plotItem.layout.addItem(self, *pos)
          self.plotItem.layout.addItem(self._oldAxis, *pos)
