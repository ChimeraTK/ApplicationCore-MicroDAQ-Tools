...
namespace uDAQ{
DataHandler::DataHandler(const std::string &folder,
                         const bool &sort, const std::vector<std::string> &matchString, const size_t &maxFiles, const std::string &treeName):
                             m_decimation(1), m_treeName(treeName), m_lastTrigger(nullptr), m_localEntry(0), m_newFile(false), m_tinfo(nullptr), m_timeStamp(nullptr){
  boost::filesystem::path p(folder);
  if(!boost::filesystem::is_directory(p))
    throw std::runtime_error("The given folder string is not a directory");
  ...
  if(sort){
    ...
    if(files.size() < 1){
      BOOST_LOG_TRIVIAL(error) << "There are no files in the Chain. Maybe the match string did not match any file." << endl;
      throw std::runtime_error("No files in the chain.");
    }
    std::sort(files.begin(), files.end());
    auto it = files.begin();
    m_chain = new TChain(m_treeName.c_str());
    if(maxFiles > 0 && maxFiles < files.size())
      it = files.end() - maxFiles;
    while(it != files.end()){
    ...
    auto it = files.begin();
    if(treeName.empty())
      m_treeName = extractTreeName(it->c_str());
    m_chain = new TChain(m_treeName.c_str());
    if(maxFiles > 0 && maxFiles < files.size())
      it = files.end() - maxFiles;
    while(it != files.end()){
...
//  m_chain->SetBranchStatus("timeInfo", 1);
  ...
  if(br){
    ...
    if(!m_timeStampBranch)
      throw std::runtime_error("Failed to read timeInfo branch");
  } else {
...
size_t DataHandler::getTypeFromTree(const char *branchName){
  ...
  if(!b)
    throw std::runtime_error(std::string("Failed to find branch: ") + branchName);

  if (!std::string(b->GetClassName()).empty()){
    if (std::string(b->GetClassName()).compare("TArrayF") == 0)
      return 1;
    if (std::string(b->GetClassName()).compare("TArrayD") == 0)
      return 2;
    if (std::string(b->GetClassName()).compare("TArrayI") == 0)
      return 3;
    if (std::string(b->GetClassName()).compare("TArrayL") == 0)
      return 4;
  ...
  if(typeName.compare("Float_t") == 0)
    return 1;
  if(typeName.compare("Double_t") == 0)
    return 2;
  if((typeName.compare("Int_t") == 0) || (typeName.compare("UInt_t") == 0))
    return 3;
  if((typeName.compare("Long64_t") == 0) || (typeName.compare("ULong64_t") == 0))
    return 4;
...
struct UpdateData{
   ...
   */
  UpdateData(DataHandler* caller, size_t nMax, size_t event, int eventID = -1): _caller(caller), _nMax(nMax), _event(event), _eventID(eventID){}
  DataHandler* _caller;
...
//    BOOST_LOG_TRIVIAL(debug) << "Size of the map: " << dataMap.size() << endl;
    for(auto it = dataMap.begin(); it!= dataMap.end(); it++){
      if(_caller->m_newFile){
        BOOST_LOG_TRIVIAL(debug) << "Switched to new file: " << _caller->m_chain->GetCurrentFile()->GetName()  << endl;
      ...
      if(it->second.branch->GetEntry(_caller->m_localEntry) <= 0){
        BOOST_LOG_TRIVIAL(error) << "Failed reading data for event: " << _event << " at local event: " << _caller->m_localEntry << ")." << endl;
        continue;
      }
...
//            throw std::runtime_error(std::string("Requested array position is too large. Maximum is :")+std::to_string(it->second.arr.GetSize()-1));
            BOOST_LOG_TRIVIAL(warning) << "Requested array position is too large. Will use maximum instead: " << it->second.arr.GetSize()-1 << endl;
        ...
        if(_caller->m_timeAxis == TimeAxis::TRUE || (_caller->m_timeAxis == TimeAxis::AUTO && !it->second.isTrace)){
...
void DataHandler::collectData(){
  ...
  if(skippedEvents > 0)
    BOOST_LOG_TRIVIAL(error) << "Skipped " << skippedEvents << " due to read errors." << endl;
  BOOST_LOG_TRIVIAL(debug) << "Collected data of " << filledEvents << " events." << endl;
  m_done = true;
}

void DataHandler::getTimeLine(const Long_t startEvent, Long_t endEvent, const size_t &decimation, const int &arrayPosition, TimeAxis timeAxis){
  m_end = endEvent;
  if (m_end == -1){
    m_end = m_nEntries;
  }
...
void DataHandler::getTriggerDecision(){
  ...
  if(!isTrace(processVariable) && m_trigger->arrayPosition !=0){
    BOOST_LOG_TRIVIAL(warning) << "Array position for process variables that are no traces should be 0 instead of " << m_trigger->arrayPosition
        << " for triggered process variable: " << processVariable << ". No search will be performed." << endl;
    return;
  }
...
void DataHandler::startTriggerSearch(std::string processVariable, const double &triggerThreshold, const std::string &triggerType, const int &arrayPosition){
...
Long_t DataHandler::findNextTrigger(const Long_t &currentEvent){
  if(m_lastTrigger->simpleSearch)
    return m_lastTrigger->nextEvent;
...
void DataHandler::startSimpleTriggerSearch(std::string processVariable, const double &triggerThreshold, const std::string &triggerType, const int &arrayPosition, const Long_t &currentEvent, const bool &increase){
...
bool DataHandler::isTrace(std::string processVariable){
  ...
  if(std::string(b->GetClassName()).empty())
    ...
    BOOST_LOG_TRIVIAL(debug) << "Process variable " << processVariable << " with type " << m_chain->GetBranch(processVariable.c_str())->GetClassName() << endl;
    return true;
  }
}
...
void DataHandler::readTimeStamp(){
  if(m_timeStamp != nullptr)
    m_timeStampBranch->GetEntry(m_localEntry);
  if(m_tinfo != nullptr)
    m_tBranch->GetEntry(m_localEntry);
}
...
std::pair<Long_t, UInt_t> DataHandler::getTimeStamp(const char* filename, std::string treeName, const Long_t &event){
  ...
  if(t == nullptr)
    throw std::runtime_error(std::string("Failed to read ") + treeName + "from file: " + filename);
  BOOST_LOG_TRIVIAL(debug) << "Opened tree: " << treeName << " from file: " << filename << " which contains " << t->GetEntries() << " entries." << endl;
  ...
  if(bTimeInfo == nullptr){
    ...
    if(bTimeInfo != nullptr)
      bTimeInfo->SetAddress(&tinfo);
  } else {
    hasTimeStamp = true;
    bTimeInfo->SetAddress(&timeStamp);
  }
  if(bTimeInfo == nullptr)
      throw std::runtime_error(std::string("Failed to read timeInfo from ") + treeName + " in file: " + filename + " which contains " + std::to_string(t->GetEntries()) + " entries!");
