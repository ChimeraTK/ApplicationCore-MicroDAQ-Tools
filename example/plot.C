...
int main(int argc, char * argv[]){
  ...
  try{
    ...
    generic.add_options()
        ("help,h",
            "This program needs the full file name.");
    generic.add_options()
        ("input,d", po::value<string>(&dir),
            "Directory with input files to be processed.");
    generic.add_options()
        ("var,v", po::value<string>(&var),
            "ProcessVariable from the TTree to be drawn.");
    generic.add_options()
        ("match,m", po::value<std::vector<string> >()->multitoken(),
            "Match string to be used for file selection");
    generic.add_options()
        ("logLevel,l", po::value<int>(&logLevel)->default_value(1),
