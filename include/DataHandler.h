...
namespace uDAQ {
  ...
  struct triggerData {
    ...
    int arrayPosition; ///< Array position used for trigger decision. If < 0 special comparisons values are compared to
                       ///< the trigger threshold: -1 (mean), -2 (max), -3 (min), -4 (any value)
    ...
    std::vector<uint>
        triggeredEvents; ///< Stored event numbers fulfilling the trigger criteria. Used when doing a complete search.
    triggerData(const triggerData&) = delete;
    triggerData(triggerData&&) = default;
    triggerData(const std::string& _processVariable, const std::string& _type, const Double_t& _threshold,
        const int& _arrayPosition, const bool& _increase = true)
    : processVariable(_processVariable), type(_type), threshold(_threshold), arrayPosition(_arrayPosition),
      increase(_increase), simpleSearch(false), nextEvent(0){};
    /**
    ...
    bool operator==(const triggerData& d) {
      ...
      if(simpleSearch || d.simpleSearch) return false;
      if(d.threshold == threshold && d.type.compare(type) == 0 && d.processVariable.compare(processVariable) == 0 &&
          d.arrayPosition == arrayPosition)
        return true;
      else
        return false;
    }
    ...
    void testValue(const Double_t& value, const Long64_t& event) {
      if(type.compare(">") == 0 && value > threshold) triggeredEvents.at(event)++;
      if(type.compare("<") == 0 && value < threshold) triggeredEvents.at(event)++;
      if(type.compare("=") == 0 && value == threshold) triggeredEvents.at(event)++;
    }

    size_t getNTrigger() { return std::accumulate(triggeredEvents.begin(), triggeredEvents.end(), 0); }
  };
  ...
  class TemplateUserTypeMap {
   public:
    boost::fusion::map<boost::fusion::pair<TArrayF, TemplateClass<TArrayF>>,
        boost::fusion::pair<TArrayD, TemplateClass<TArrayD>>, boost::fusion::pair<TArrayI, TemplateClass<TArrayI>>,
        boost::fusion::pair<TArrayL, TemplateClass<TArrayL>>, boost::fusion::pair<TArrayS, TemplateClass<TArrayS>>,
  ...
  class DataHandler {
   protected:
    ...
    /**
     * Find the file in the chain that includes event and set m_localEntry, which is the corresponding event number in
     * the file.
     *
    ...
    void setupBranch(const std::string& branchName) {
      ...
      BOOST_LOG_TRIVIAL(info) << "Adding branch " << branchName.c_str()
                              << " to list of active branches (isTrace: " << isTrace << ")." << std::endl;
   ...
   public:
     ...
     * \param sort If true root files are opened and the timestamp of the first event is extracted. Input files are
     * sorted afterwards by this timestamp. \param matchString If set only files matching the given string are
     * considered. This can be used to work on a subset of files. \param maxFiles Limit the number of files to be
     * processed. Files are sorted by name or by timestamp and only the latest files are considered \param treeName The
     * name of the TTree holding the data.
     */
    DataHandler(const std::string& folder, const bool& sort = true,
        const std::vector<std::string>& matchString = std::vector<std::string>(), const size_t& maxFiles = 0,
     ...
     *
     * \remark In order to set branches the process variables names are changes: /Probe/Test -> .Probe.Test. The
     * chainHelper uses the PV names.
     */
     ...
     * \param arrayPosition The element in arrays that is put into the trace. If it is -1 the average of the array is
     * considered. If it is -2 the maximum is considered. If it is -3 the minimum is considered. \param TimeAxis If true
     * the x axis is filled time stamps. Else index is used for the x axis.
     *
     */
    void getTimeLine(const Long_t startEvent = 0, Long_t endEvent = -1, const size_t& decimation = 1,
        const int& arrayPosition = 0, TimeAxis = TimeAxis::TRUE);
     ...
     * \param currentEvent The trigger is searched starting at the given event. Events before that event are not
     * considered. \param triggerThreshold The threshold used to search for events \param triggerType This should be an
     * operator known to ROOT (e.g. >, <, ==) \param arrayPosition If an position is given only the corresponding
     * position in arrays is considered in the trigger decision
     */
    void startTriggerSearch(std::string processVariable, const double& triggerThreshold, const std::string& triggerType,
     ...
     * considered. \param triggerThreshold The threshold used to search for events \param triggerType This should be an
     * operator known to ROOT (e.g. >, <, ==) \param arrayPosition If an position is given only the corresponding
     * position in arrays is considered in the trigger decision \param currentEvent Event where to start the trigger
     * search \param increase Define the search direction \return The event number of the next triggered event. If no
     * event was trigger -1 is returned.
     */
    void startSimpleTriggerSearch(std::string processVariable, const double& triggerThreshold,
     ...
     * This list is similar to the process variables in the original h5 files, except that the delimiter is a dot
     * instead of a slash. Furthermore, it includes the time information branch.
     */
     ...
     * \remark Depending on the available information in the Root file either m_tinfo (deprecated and written by
     * hdf5converter) or m_timeStamp (written by ctk::RootDAQ) will be filled.
     */
